...
def main():
    parser = argparse.ArgumentParser(description='Программа помогает сокрашать ссылки а так же получать количество переходов по сокращенной ссылке')
    parser.add_argument('--url', help='Введите ссылку:')
    args = parser.parse_args()
    parsed_url = urlparse(args.url)
    url_parse = f"{parsed_url.netloc}{parsed_url.path}"
    load_dotenv()
    bitly_key = os.environ['BITLY_KEY']
    try:
        if is_bitlink(bitly_key,url_parse):
            print(count_clicks(bitly_key,url_parse))
        else:
